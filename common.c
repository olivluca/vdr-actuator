/*
 * common.c: actuator - A plugin for the Video Disk Recorder
 *
...
void _log(const char* function, int line, const int level, bool newline, const char* fmt, ...) {
  if (/*level > wSetup.verbosity*/false)
     return;
...
void TChannel::CopyTransponderData(const TChannel* Channel) {
  //if (Frequency == 11200)
  //dlog(4,"Modifying transponder %d",this);
  if (Channel) {
...
bool TChannel::ValidSatIf() {
  ...
  if (Setup.DiSEqC) {
     ...
     for(d = Diseqcs.First(); d; d = Diseqcs.Next(d))
        if (cSource::Matches(d->Source(),cSource::FromString(Source.c_str())) and d->Slof() > f and d->Polarization() == Polarization) {
           f -= d->Lof();
           break;
           }
     if (!d) {
